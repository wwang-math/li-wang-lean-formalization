...
theorem WB.conv_tame {r : ℕ} {a b : Gam → ℂ} {Ra R0a Rb R0b : ℝ}
    ...
    WB r (2 ^ r * (Ra * R0b + R0a * Rb)) (_root_.LiWang.Formalization.convFun a b) := by
  classical
  ...
  have hkey : ∀ k : Gam, wt k ^ r * ‖_root_.LiWang.Formalization.convFun a b k‖ ≤ 2 ^ r * (P k + Q k) := by
    intro k
    ...
    calc wt k ^ r * ‖_root_.LiWang.Formalization.convFun a b k‖
        ≤ wt k ^ r * ∑' p, ‖a p * b (k - p)‖ :=
  ...
  intro F
  have hstep1 : (∑ k ∈ F, wt k ^ r * ‖_root_.LiWang.Formalization.convFun a b k‖)
      ≤ ∑ k ∈ F, 2 ^ r * (P k + Q k) :=
  ...
  have hstep4 : (∑' k : Gam, (P k + Q k)) ≤ Ra * R0b + R0a * Rb := by
    ...
    linarith [hPt, hQt]
  calc (∑ k ∈ F, wt k ^ r * ‖_root_.LiWang.Formalization.convFun a b k‖)
