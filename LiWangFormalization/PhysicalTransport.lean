...
`LiWang.Formalization.torusConv` (`IntegrableKernel.lean`), and `LiWang.Formalization.torusConv_synth` proves that it
agrees with this coefficient law on synthesized states; `LiWang.Formalization.symbolConv_eq_kernelConv`
