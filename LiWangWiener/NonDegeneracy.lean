...
`LiWang.WienerModel.testSymbolNoI_reality_failure` in Section 12.  It is retained here only as a
...
`LiWang.WienerModel.rotatedGradientSymbol` in Section 12; the showcased concrete example of the
packet is `LiWang.WienerModel.exampleKernel` in Section 14. -/
...
see the warning above and `LiWang.WienerModel.testSymbolNoI_reality_failure`. -/
